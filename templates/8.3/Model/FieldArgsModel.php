 ...
 * PHP representation of graphql field args of <?=$type_name?>.<?=$field_name?><?=PHP_EOL?>
<?php if (isset($type_description)): ?>
 * <?=$type_description?>
<?php endif ?>
<?php foreach ($fields ?? [] as $field): ?>
 * @property <?php if ($field['is_list']): ?>array<mixed><?php elseif (!empty($field['type_doc'])): ?><?=$field['type_doc']?><?php else: ?>mixed<?php endif ?> $<?=$field['name']?><?php if ($field['is_nullable']): ?> = null<?php endif ?> <?=$field['description']?><?=PHP_EOL?>
<?php endforeach ?>
 */
...
<?php if ($field['is_list'] && $field['is_custom'] && $field['type']): ?>
        if ($name === '<?=$field['name']?>') {
            return <?=$listRender('($v === null ? null : new ' . $field['type'] . '($v));', $field['list_level'])?><?=PHP_EOL?>
        }
